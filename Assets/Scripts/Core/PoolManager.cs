...
public class EnemyPoolAdapter : PoolManager.IPoolMonitorable
{
    private EnemyPool pool;
    
    public EnemyPoolAdapter(EnemyPool pool)
    {
        this.pool = pool;
    }
    
    public string PoolName => "EnemyPool";
    public int TotalCount => pool.TotalPooledEnemies;
    public int ActiveCount => pool.TotalActiveEnemies;
    public int AvailableCount => TotalCount - ActiveCount;
    public float EstimatedMemoryUsage => TotalCount * 0.5f; // 估算每个敌人500KB
    public bool CanOptimize => true;
    
    public void OptimizePool(int newSize)
    {
        Debug.Log($"[EnemyPool] 优化建议: 目标大小={newSize}");
    }
    
    public void PreloadPool(int count)
    {
        pool.PreloadAllEnemies(count);
    }
...
public class SwordAfterimagePoolAdapter : PoolManager.IPoolMonitorable
{
    private SwordAfterimagePool pool;
    
    public SwordAfterimagePoolAdapter(SwordAfterimagePool pool)
    {
        this.pool = pool;
    }
    
    public string PoolName => "SwordAfterimagePool";
    public int TotalCount => pool.TotalCount;
    public int ActiveCount => pool.ActiveCount;
    public int AvailableCount => pool.AvailableCount;
    public float EstimatedMemoryUsage => TotalCount * 0.02f; // 估算每个残影20KB
    public bool CanOptimize => true;
    
    public void OptimizePool(int newSize)
    {
        Debug.Log($"[SwordAfterimagePool] 优化建议: 目标大小={newSize}");
    }
    
    public void PreloadPool(int count)
    {
        pool.PreloadAfterimages(count);
    }
